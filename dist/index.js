...
class ErrorBoundary extends React__default['default'].Component {
    ...
    componentDidMount() {
        ...
        window.addEventListener('error', (error) => {
            ...
            try {
                ...
                const { performance, clientInformation, location, screen, } = info.target;
                const obj = {
                    caughtEvent: 'onerror',
        ...
        window.addEventListener('unhandledrejection', (error) => {
            try {
                const info = pick_1(error, ['type', 'target', 'reason', 'isTrusted']);
                const { message, stack } = info.reason;
                const { performance, clientInformation, location, screen, } = info.target;
                const obj = {
    ...
    render() {
        ...
        if (this.state.hasError) {
            return errorRender ? React__default['default'].createElement(React__default['default'].Fragment, null, errorRender) : React__default['default'].createElement("h1", null, "Something went wrong.");
        }
